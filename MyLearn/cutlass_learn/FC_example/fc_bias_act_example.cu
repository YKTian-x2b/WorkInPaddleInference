...
int run(std::string desc, Options &options){
    ...
    std::cout << "================" << std::endl;
    std::cout << options.tensor_A.layout().stride(0) << std::endl;
    std::cout << std::is_same<LayoutA, cutlass::layout::RowMajor>::value << std::endl;
}
